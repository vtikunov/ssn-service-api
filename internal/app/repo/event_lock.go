...
func (r *eventRepo) Lock(ctx context.Context, n uint64, tx QueryerExecer) ([]subscription.ServiceEvent, error) {
	...
	subQ1 := sq.Select("s.id").From("service_events s").PlaceholderFormat(sq.Dollar)
	subQ1 = subQ1.LeftJoin("service_events s1 on (s1.service_id = s.service_id AND s1.status = ?)", subscription.Processed)
	subQ1 = subQ1.Where(sq.And{sq.Expr("s1.service_id IS NULL"), sq.Eq{"s.status": subscription.Deferred}})
	subQ1 = subQ1.OrderBy("s.id").Limit(n)
	...
	query = query.Where(subQ1.Prefix("id IN (").Suffix(")"))
